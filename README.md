...
```
 1. 売買すべき銘柄のスクリーニング～通知機能（すべての条件を満たす銘柄ではなく、スコアリングし、60点以上の銘柄を通知する）
      * 共通条件
         * TOPIXの状況確認
            * TOPIXがステージ1,6、且つ、3本のEMAの傾きがプラスの時のみ買いでエントリー
            * TOPIXがステージ3,4、且つ、3本のEMAの傾きがマイナスの時のみ売りでエントリー
         * 市場選択
         * 予算
         * 出来高が極端に少ない銘柄は回避する。
 ...
 2.  利確すべきタイミングの条件～通知
      * 売りのタイミング
         1. 固定利幅(3ATR&1の位を四捨五入したキリの良い数字)で即利確※絶対条件
      ...
      * 下記よりポイント制とし、70点以上で利確
         ...
         7. 出来高の急減(20点)
            * 本日の出来高が直近5日間の平均出来高の半分である場合（10点）
            * 本日の出来高が前日の出来高の半分になった場合(10点)



      * 買い戻し（返済注文）のタイミング

         1. 目標利益幅（リスクリワード比）に到達（絶対条件）
         定義：エントリーポイントから3ATR以上下落、且つ、1の位を四捨五入
         ```
         計算例：
         空売りエントリー価格：1,000円
         1ATR：21円 → 利確目標価格 = 1,000 - (21 × 3) = 937円　⇒ 940円
         ```

         2. 価格が下降トレンド中に一定割合戻したら利確（トレーリングストップ）
         
         * 空売り後の最安値を最大利益点と定義する。以降、それを更新した場合は更新する。
         * また、ATRは空売りした時点の断面を保持しておく。
         * 現在価格 = 最大利益点 + 1ATR以上となった場合、返済
